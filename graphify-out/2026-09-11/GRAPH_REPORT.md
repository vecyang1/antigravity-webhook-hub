...
## Corpus Check
- 239 files · ~319,086 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2675 nodes · 4196 edges · 273 communities (186 shown, 87 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 64 edges (avg confidence: 0.57)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `572d278a`
...
- dashboard.py
- Namespace
- test_agent_activities_routes.py
- .sweep_unprocessed_tasks
- .route
- ._pressure_relief
- RoutePattern
- cmd_logs
...
1. `DatabaseManager` - 107 edges
2. `AsyncHTTPServer` - 95 edges
3. `AppConfig` - 81 edges
4. `TaskDispatcher` - 64 edges
5. `generate_hmac_signature()` - 61 edges
6. `EventBroker` - 50 edges
7. `ContactInput` - 44 edges
8. `register_observability_routes()` - 39 edges
9. `build_parser()` - 35 edges
10. `main()` - 35 edges
...
Cohesion: 0.14
Nodes (19): apply_memory_pressure_relief(), _get_darwin_resident_bytes(), get_memory_budget_mb(), get_memory_rss_bytes(), get_memory_rss_mb(), _MachTaskBasicInfo, Any, Antigravity Webhook Hub — Unified Memory Management & Footprint Controller.… (+11 more)

### Community 1 - "Requirement R1 Technical Survey & Architectural Report"
Cohesion: 0.05
Nodes (37): 1. Executive Summary & Core Architectural Recommendations, 2.1 Framework Evaluation & Empirical Benchmarks, 2.2 Detailed Memory Profiling of Imports, 2.3 Proposed Async HTTP/1.1 Engine Implementation Pattern, 2. Lightweight HTTP Server Architecture & Empirical Benchmarking, 3.1 Security Architecture Overview, 3.2 HMAC SHA-256 Signature Protocol, 3.3 Replay Attack Protection: Dual-Layer Defense (+29 more)

### Community 2 - "StressTestRunner"
Cohesion: 0.10
Nodes (17): Test client declaring Content-Length: 100 but sending only 10 bytes then…, Test server shutdown behavior when a client stalls on sending body., Test Content-Length exceeding max_body_bytes returns 413., Test chunked payload exceeding max_body_bytes returns 413., Test large payload within limits (3900 bytes for 4096 limit)., Test header exceeding StreamReader limit (64KB). Server should close connection…, Test request lines with 0, 1, 2, or >3 parts., Test invalid Content-Length headers. (+9 more)

### Community 3 - "Technical Architecture & Investigation Report: Requirement R2 (Single-Source-of-Truth Task State & Dispatch Engine)"
Cohesion: 0.06
...
Cohesion: 0.06
Nodes (32): 1. Executive Summary & Architectural Invariants, 1. `/healthz` (Liveness & Readiness Probe), 1. Legitimate Path Test Cases, 2.1 Server-Sent Events (SSE) Streaming Architecture, 2.2 Observability Endpoints Specification, 2. Adversarial Challenge Test Cases, 2. `/metrics` (Prometheus & Observability Endpoint), 2. Requirement R3: Real-Time Observability & Push-Based Live Queries (+24 more)

### Community 6 - "AuthHelper"
Cohesion: 0.09
...
Cohesion: 0.07
Nodes (52): DashboardConfig, ObservabilityConfig, Antigravity Webhook Hub — Clean Configuration Loader Zero hardcoded credentials…, SecurityConfig, ServerConfig, TunnelConfig, HTTPRequest, HTTPResponse (+44 more)
...
Cohesion: 0.11
Nodes (21): generate_hmac_signature(), Generate an HMAC SHA-256 signature binding timestamp and raw body. Algorithm:…, Any, Tier 2: Replay attack with timestamp 350s old rejected with 401 and ZERO DB…, Tier 2: Future timestamp 90s in future rejected with 401 and ZERO DB writes., Tier 2: 5 rapid duplicate requests produce exactly 1 task in SQLite., Tier 2: Process group is cleanly killed on timeout without hanging gateway., Tier 2: Tampered HMAC signature rejected with 401 and ZERO DB writes. (+13 more)

### Community 11 - "cmd_sweep"
Cohesion: 0.12
Nodes (18): cmd_review_contact(), cmd_start(), cmd_sweep(), Path, Read integer PID from PID file if it exists and is valid., Handle `webhook-hub start` command., Handle `webhook-hub review-contact` command., Handle `webhook-hub sweep` (or `pick-unprocessed`) command. (+10 more)

### Community 12 - ".close"
Cohesion: 0.10
Nodes (16): Any, Atomically insert webhook event and associated task in a single transaction.…, Retrieve chronological execution logs for a task., Run read-only query under lock without ThreadPool overhead., Run write query serialized under lock., Run write query with RETURNING clause under lock., Cleanly close SQLite connection., Insert incoming webhook event into webhook_events table. Returns True on… (+8 more)
...
Cohesion: 0.08
Nodes (27): cmd_stop(), _is_pid_running(), Handle `webhook-hub stop` command., Check if process with given PID exists and is running., Unit tests for Antigravity Webhook Hub Unified Command Line Interface…, Verify sweep subcommand and aliases are registered., Verify all 6 subcommands are registered with appropriate options., Verify `webhook-hub dashboard` command outputs local and tunnel URLs. (+19 more)
...
Cohesion: 0.08
Nodes (35): EventBroker, Antigravity Webhook Hub — In-Memory Bounded PubSub Event Broker Provides fast,…, In-memory bounded PubSub event bus. Routes events to task-specific subscribers…, Clear all active subscriptions., Return total active subscribers or active subscribers on a topic., Antigravity Webhook Hub — SQLite SSOT Database Engine Provides single-source-…, Antigravity Webhook Hub — Async Task Dispatcher Engine Decoupled async worker…, Antigravity Webhook Hub — Observability & Query Endpoints Implements /healthz,… (+27 more)

### Community 18 - "Milestone M1 Gate Review & Adversarial Audit Report"
Cohesion: 0.12
Nodes (15): 1.1 Integrity Check, 1.2 Independent Build and Test Execution, 1.3 Adversarial Stress Testing Results, 1.4 Observed Defect in Tunnel Scripts, 1. Observation, 2. Logic Chain, 3. Findings, 4. Caveats (+7 more)

### Community 19 - "1. Observation"
Cohesion: 0.13
Nodes (14): 1.1 Summary of Test Results, 1.2 Defect 1 (CRITICAL): Server Graceful Shutdown Deadlock on Active Connections, 1.3 Defect 2 (HIGH): Negative Content-Length Accepted as Valid Empty Body, 1.4 Defect 3 (MEDIUM / SECURITY): Missing Timeout on Request Body Reading (Slow POST DoS), 1.5 Defect 4 (MEDIUM / SECURITY): HTTP Request Smuggling Risk on Conflicting CL and TE, 1.6 Cloudflare Tunnel Scripts & SO_REUSEADDR Observations, 1. Observation, 2. Logic Chain (+6 more)

### Community 20 - "Milestone M2 Empirical Challenge Report: SQLite SSOT & Async Dispatch Engine"
Cohesion: 0.13
Nodes (14): 1. Observation, 2. Logic Chain, 3. Caveats, 4. Conclusion, 5. Verification Method, A. Baseline Test Execution, Actionable Remediation Items for Worker M2:, B. Empirical Defect Discoveries & Verifications (+6 more)

### Community 21 - "Empirical Challenger M2-2 Verification Report: Async Dispatcher Engine"
Cohesion: 0.13
Nodes (14): 1.1 Empirical Stress Test Harness Results, 1.2 Baseline Unit and Integration Tests, 1.3 Standalone End-to-End Verification, 1. Observation, 2.1 Subprocess Timeout and Process Group Isolation (`hub/dispatcher.py:288-361`), 2.2 Error Handling and SSOT Persistence (`hub/dispatcher.py:321-385`, `hub/db.py:392-435`), 2.3 Antigravity Agent Signal Emission (`hub/dispatcher.py:218-251`), 2.4 Memory RSS Analysis & Budget Headroom (`hub/config.py`, `hub/db.py`) (+6 more)

### Community 22 - "Acceptance Criteria"
Cohesion: 0.13
...
### Community 23 - "1. Observation"
Cohesion: 0.13
Nodes (14): 1.1 Test Suite & Verification Execution, 1.2 SQLite PRAGMA & Schema Verification (`hub/db.py`), 1.3 State Machine Atomic CAS Transitions (`hub/db.py` & `hub/dispatcher.py`), 1.4 Zero Database Writes on Ingress Rejections (`hub/routes/webhook.py`), 1.5 Async Subprocess Execution & Process Group Kill (`hub/dispatcher.py`), 1. Observation, 2. Logic Chain, 3. Caveats & Adversarial Findings (+6 more)

### Community 24 - "Acceptance Criteria"
Cohesion: 0.13
Nodes (14): Acceptance Criteria, Automated Testing, Initial Request — 2026-09-08T10:27:16Z, Original User Request, Performance & System Impact, R1. Minimal-Footprint Webhook Ingress Gateway, R2. Single-Source-of-Truth Task State & Dispatch Engine, R3. Real-Time Observability & Push-Based Live Queries (+6 more)
...
Cohesion: 0.14
Nodes (15): ExecutionResult, Result of a dispatched task execution attempt., ChallengerTestRunner, main(), 1.2: Command spawns child background processes -> verify entire process group…, 1.3: Subprocess ignores SIGTERM -> verify escalation to SIGKILL within timeout., 2.1: Non-zero exit code -> verify exit code in DB, stderr in execution_logs,…, 2.2: Non-existent executable -> shell returns 127, status failed, error logged. (+7 more)
...
Cohesion: 0.17
Nodes (11): Artifact Index, BRIEFING — 2026-09-08T10:43:00Z, Change Tracker, Current Parent, 🔒 Key Constraints, Key Decisions Made, Loaded Skills, Mission (+3 more)

### Community 32 - "BRIEFING — 2026-09-08T11:08:00Z"
Cohesion: 0.17
Nodes (11): Artifact Index, BRIEFING — 2026-09-08T11:08:00Z, Change Tracker, Current Parent, 🔒 Key Constraints, Key Decisions Made, Loaded Skills, Mission (+3 more)

### Community 33 - "BRIEFING — 2026-09-08T18:15:00+07:00"
Cohesion: 0.17
Nodes (11): Artifact Index, BRIEFING — 2026-09-08T18:15:00+07:00, Change Tracker, Current Parent, 🔒 Key Constraints, Key Decisions Made, Loaded Skills, Mission (+3 more)

### Community 34 - "BRIEFING — 2026-09-08T10:44:30Z"
Cohesion: 0.18
Nodes (10): Artifact Index, Attack Surface, BRIEFING — 2026-09-08T10:44:30Z, Current Parent, 🔒 Key Constraints, Key Decisions Made, Loaded Skills, Mission (+2 more)

### Community 35 - "BRIEFING — 2026-09-08T11:01:00Z"
Cohesion: 0.18
Nodes (10): Artifact Index, Attack Surface, BRIEFING — 2026-09-08T11:01:00Z, Current Parent, 🔒 Key Constraints, Key Decisions Made, Loaded Skills, Mission (+2 more)

### Community 36 - "BRIEFING — 2026-09-08T11:17:30Z"
Cohesion: 0.18
Nodes (10): Artifact Index, Attack Surface, BRIEFING — 2026-09-08T11:17:30Z, Current Parent, 🔒 Key Constraints, Key Decisions Made, Loaded Skills, Mission (+2 more)

### Community 37 - "BRIEFING — 2026-09-08T11:23:55Z"
Cohesion: 0.18
Nodes (10): Artifact Index, Attack Surface, BRIEFING — 2026-09-08T11:23:55Z, Current Parent, 🔒 Key Constraints, Key Decisions Made, Loaded Skills, Mission (+2 more)

### Community 38 - "BRIEFING — 2026-09-08T10:49:00Z"
Cohesion: 0.18
Nodes (10): Artifact Index, Attack Surface, BRIEFING — 2026-09-08T10:49:00Z, Current Parent, 🔒 Key Constraints, Key Decisions Made, Mission, 🔒 My Identity (+2 more)

### Community 39 - "BRIEFING — 2026-09-08T10:50:00Z"
Cohesion: 0.18
Nodes (10): Artifact Index, Attack Surface, BRIEFING — 2026-09-08T10:50:00Z, Current Parent, 🔒 Key Constraints, Key Decisions Made, Mission, 🔒 My Identity (+2 more)

### Community 40 - "Milestone M1 (Ingress Gateway & Security Core) — Review & Adversarial Critic Report"
Cohesion: 0.18
Nodes (10): 1.1 Test Execution Observations, 1.2 Independent Empirical Memory & CPU Benchmark Observations, 1.3 Code Inspection Observations, 1. Observation, 2. Logic Chain, 3. Caveats, 4. Conclusion, 5. Verification Method (+2 more)

### Community 41 - "BRIEFING — 2026-09-08T11:21:00Z"
Cohesion: 0.18
Nodes (10): Artifact Index, Attack Surface, BRIEFING — 2026-09-08T11:21:00Z, Current Parent, 🔒 Key Constraints, Key Decisions Made, Mission, 🔒 My Identity (+2 more)

### Community 42 - "BRIEFING — 2026-09-08T11:21:50Z"
Cohesion: 0.18
Nodes (10): Artifact Index, Attack Surface, BRIEFING — 2026-09-08T11:21:50Z, Current Parent, 🔒 Key Constraints, Key Decisions Made, Mission, 🔒 My Identity (+2 more)

### Community 43 - "BRIEFING — 2026-09-08T10:34:35Z"
Cohesion: 0.18
Nodes (10): Artifact Index, BRIEFING — 2026-09-08T10:34:35Z, Current Parent, 🔒 Key Constraints, Key Decisions Made, Loaded Skills, Mission, 🔒 My Identity (+2 more)

### Community 44 - "E2E Test Infra: Antigravity Webhook Hub"
Cohesion: 0.18
Nodes (10): E2E Test Infra: Antigravity Webhook Hub, Feature Inventory & Test Coverage Matrix, Minimum Acceptance Criteria, Test Architecture, Test Philosophy, Test Tiers Definition, Tier 1: Feature Coverage (Happy Path Isolation), Tier 2: Boundary & Corner Cases (Adversarial Path Isolation) (+2 more)

### Community 45 - "E2E Test Suite Readiness Report: Antigravity Webhook Hub"
Cohesion: 0.18
...
Cohesion: 0.20
Nodes (9): Architecture, Code Layout, Dispatcher ↔ Broker / SSE, Feature Inventory, Ingress ↔ Database, Ingress ↔ Dispatcher, Interface Contracts, Milestones (+1 more)

### Community 52 - "BRIEFING — 2026-09-08T10:33:30Z"
Cohesion: 0.22
Nodes (8): Artifact Index, BRIEFING — 2026-09-08T10:33:30Z, Current Parent, Investigation State, 🔒 Key Constraints, Key Decisions Made, Mission, 🔒 My Identity

### Community 53 - "BRIEFING — 2026-09-08T10:29:30Z"
Cohesion: 0.22
Nodes (8): Artifact Index, BRIEFING — 2026-09-08T10:29:30Z, Current Parent, Investigation State, 🔒 Key Constraints, Key Decisions Made, Mission, 🔒 My Identity

### Community 54 - "BRIEFING — 2026-09-08T10:31:30Z"
Cohesion: 0.22
Nodes (8): Artifact Index, BRIEFING — 2026-09-08T10:31:30Z, Current Parent, Investigation State, 🔒 Key Constraints, Key Decisions Made, Mission, 🔒 My Identity

### Community 55 - "Milestone M1 Remediation Handoff Report"
Cohesion: 0.22
Nodes (8): 1.1 Pre-Remediation Baseline, 1.2 Remediated Codebase Changes, 1. Observation, 2. Logic Chain, 3. Caveats, 4. Conclusion, 5. Verification Method, Milestone M1 Remediation Handoff Report

### Community 56 - "Any"
Cohesion: 0.22
Nodes (9): Any, Tier 3: Multiple concurrent incoming webhooks under load with SQLite WAL., Tier 4: GitHub push simulation triggering local build script., Tier 4: Cloudflare Tunnel simulated headers (CF-Connecting-IP, CF-Ray,…, Tier 3: Ingress -> DB -> Async Dispatch -> Execution -> Final Query., test_concurrent_webhook_deliveries(), test_full_webhook_lifecycle_e2e(), test_scenario_github_webhook_simulation() (+1 more)

### Community 57 - "Python Testing Patterns"
Cohesion: 0.25
Nodes (7): Assertions, Basic Test Structure, pytest Fundamentals, Python Testing Patterns, Quick Reference, Running Tests, When to Activate

### Community 58 - "Fixtures"
Cohesion: 0.25
...
### Community 59 - "Handoff Report: Requirement R1 (Minimal-Footprint Webhook Ingress Gateway)"
Cohesion: 0.29
Nodes (6): 1. Observation, 2. Logic Chain, 3. Caveats, 4. Conclusion, 5. Verification Method, Handoff Report: Requirement R1 (Minimal-Footprint Webhook Ingress Gateway)

### Community 60 - "Handoff Report: Requirement R2 (Single-Source-of-Truth Task State & Dispatch Engine)"
Cohesion: 0.29
Nodes (6): 1. Observation, 2. Logic Chain, 3. Caveats, 4. Conclusion, 5. Verification Method, Handoff Report: Requirement R2 (Single-Source-of-Truth Task State & Dispatch Engine)

### Community 61 - "Handoff Report: Survey of Requirements R3, R4, and R5"
Cohesion: 0.29
Nodes (6): 1. Observation, 2. Logic Chain, 3. Caveats, 4. Conclusion, 5. Verification Method, Handoff Report: Survey of Requirements R3, R4, and R5

### Community 62 - "Handoff Report — Test Writer T1 (E2E Testing Track)"
Cohesion: 0.29
Nodes (6): 1. Observation, 2. Logic Chain, 3. Caveats, 4. Conclusion, 5. Verification Method, Handoff Report — Test Writer T1 (E2E Testing Track)

### Community 63 - "Milestone M1 (Ingress Gateway & Security Core) — Handoff Report"
Cohesion: 0.29
Nodes (6): 1. Observation, 2. Logic Chain, 3. Caveats, 4. Conclusion, 5. Verification Method, Milestone M1 (Ingress Gateway & Security Core) — Handoff Report

### Community 64 - "Milestone M2 Handoff Report: SQLite SSOT & Async Dispatch Engine"
Cohesion: 0.29
...
### Community 67 - "RouteHandler"
Cohesion: 0.20
Nodes (10): _extract_handler_params(), _get_invoker(), _HandlerInvoker, _is_coroutine_callable(), Any, Pre-analyzed dispatch invoker for a route handler to avoid runtime inspect…, Retrieve or attach cached invoker directly on handler to prevent unbounded…, Register a handler for an HTTP method and path. (+2 more)
...
Cohesion: 0.05
Nodes (47): _LightweightEmailUtils, _LightweightSSL, _StartArgsNamespace, AppConfig, DatabaseConfig, DispatchConfig, Validate loaded config and return any errors or warnings., validate_config() (+39 more)

### Community 70 - "Orchestrator Progress"
Cohesion: 0.40
Nodes (4): Active Subagents, Current Status, Iteration Status, Orchestrator Progress

### Community 71 - "Parametrization"
Cohesion: 0.40
Nodes (5): Basic Parametrization, Multiple Parameters, Parametrization, Parametrize with IDs, Parametrized Fixtures

### Community 72 - "start_tunnel.sh"
Cohesion: 0.70
...
### Community 90 - "register_observability_routes"
Cohesion: 0.10
Nodes (33): Run gateway server in the foreground with full subsystem wiring., run_server_foreground(), Register Webhook Hub Dashboard routes: GET /dashboard and GET /ui., register_dashboard_routes(), __getattr__(), Any, Antigravity Webhook Hub — Routes Package Implements PEP 562 lazy module loading…, Any (+25 more)
...
Cohesion: 0.15
Nodes (12): 1.1 Source Inspection, 1.2 Tool Commands and Direct Results, 1. Observation, 2. Logic Chain, 3. Findings, 4. Adversarial Stress Test Results, 5. Caveats, 6. Conclusion (+4 more)

### Community 119 - ".publish"
Cohesion: 0.17
Nodes (7): Any, Forward a stdout/stderr log chunk to subscribers., Forward a task status change event to subscribers., Forward a generic event to subscribers., Query execution_logs from database for log records with log_id > since_id., Query webhook_events from database for backlog replay., Fast, non-blocking broadcast of data to all matching subscriber queues. Drops…

### Community 120 - "1. Observation"
Cohesion: 0.17
Nodes (11): 1.1 Concurrent SSE Subscribers (Challenge 1), 1.2 Slow Consumer & Overflow (Challenge 2), 1.3 Sudden Disconnect Cleanup (Challenge 3), 1.4 Memory RSS Under Active Streaming (Challenge 4), 1.5 Peer Failure in Task Detail Logs (`hub/routes/observability.py:264-288`), 1. Observation, 2. Logic Chain, 3. Caveats (+3 more)

### Community 127 - "BRIEFING — 2026-09-08T18:30:00+07:00"
Cohesion: 0.17
Nodes (11): Artifact Index, BRIEFING — 2026-09-08T18:30:00+07:00, Change Tracker, Current Parent, 🔒 Key Constraints, Key Decisions Made, Loaded Skills, Mission (+3 more)

### Community 128 - "BRIEFING — 2026-09-08T11:39:10Z"
Cohesion: 0.17
...
Cohesion: 0.18
Nodes (10): Artifact Index, Attack Surface, BRIEFING — 2026-09-08T11:40:00Z, Current Parent, 🔒 Key Constraints, Key Decisions Made, Loaded Skills, Mission (+2 more)

### Community 132 - "BRIEFING — 2026-09-08T11:45:00Z"
Cohesion: 0.18
Nodes (10): Artifact Index, Attack Surface, BRIEFING — 2026-09-08T11:45:00Z, Current Parent, 🔒 Key Constraints, Key Decisions Made, Loaded Skills, Mission (+2 more)

### Community 133 - "BRIEFING — 2026-09-08T11:45:00Z"
Cohesion: 0.18
Nodes (10): Artifact Index, Attack Surface, BRIEFING — 2026-09-08T11:45:00Z, Current Parent, 🔒 Key Constraints, Key Decisions Made, Mission, 🔒 My Identity (+2 more)

### Community 134 - "BRIEFING — 2026-09-08T11:44:00Z"
Cohesion: 0.18
...
Cohesion: 0.20
Nodes (9): 1.1 Tool Commands and Results, 1.2 Direct Code Inspections, 1. Observation, 2. Logic Chain, 3. Caveats, 4. Conclusion, 5. Verification Method, Empirical Challenge Report: Milestone M3 (Observability & Backlog Replay) (+1 more)

### Community 137 - "Milestone M2 Remediation Report: SQLite SSOT & Async Dispatch Engine"
Cohesion: 0.20
Nodes (9): 1. Observation, 2. Logic Chain, 3. Caveats, 4. Conclusion, 5. Verification Method, A. Initial Defect Observations, B. Remediation Verification Results, Invalidation Conditions: (+1 more)

### Community 138 - "Milestone M3 Reviewer Report: Real-Time Observability & Push-Based Live Queries"
Cohesion: 0.22
...
Cohesion: 0.29
Nodes (6): 1. Observation, 2. Logic Chain, 3. Caveats, 4. Conclusion, 5. Verification Method, Milestone M3 Handoff Report: Real-Time Observability & Push-Based Live Queries

### Community 141 - "DatabaseManager"
Cohesion: 0.11
Nodes (26): SweeperConfig, DatabaseManager, Create 4-table relational schema and corresponding performance indexes., SQLite Single-Source-of-Truth (SSOT) Database Manager. Enforces WAL mode,…, asyncio, Unit & integration tests for background sweeper, unprocessed task recovery, and…, Simulate sleep clock jump triggering recovery., Verify orphaned events with non-dict JSON or raw strings rehydrate cleanly… (+18 more)

### Community 142 - "Progress Tracker — Worker M2 Remediation"
Cohesion: 0.50
...
Cohesion: 0.20
Nodes (9): [1.3.0] - 2026-09-10, [1.3.4] - 2026-09-10, [1.4.1] - 2026-09-11, [1.6.2] - 2026-09-11, Added, Changelog, Fixed, Fixed (+1 more)
...
Cohesion: 0.20
Nodes (10): cmd_status(), _get_process_rss_mb(), Query exact resident set size (RSS) of a PID using ps on macOS/Linux., Handle `webhook-hub status` command., Verify status reports stopped when PID file does not exist., Verify status detects dead PID., Verify status --json outputs valid JSON metrics when server responds., test_cmd_status_server_running_json() (+2 more)

### Community 224 - "hub/__init__.py"
Cohesion: 0.18
Nodes (6): __getattr__(), _LightweightEmailUtils, _LightweightSSL, Any, Antigravity Webhook Hub A lightweight, zero-footprint local webhook gateway and…, Lazy-load public package attributes on access (PEP 562) to maintain strict…

### Community 225 - "cli.py"
Cohesion: 0.29
Nodes (19): ArgumentParser, _add_dashboard_args(), _add_logs_args(), _add_rerun_args(), _add_review_contact_args(), _add_send_args(), _add_start_args(), _add_status_args() (+11 more)

### Community 226 - "Any"
Cohesion: 0.09
Nodes (24): Any, Path, Verify GET / returns HTML for browser navigation and JSON for machine API calls., Verify GET /tasks/summary and GET /activities/summary derive stats from SQLite…, Verify /tasks search query q, status filter, and source filter., Verify POST /tasks/{id}/rerun resets task to queued and notifies dispatcher., Verify task dispatcher emits Antigravity sidecar activity events for agent…, Verify task dispatcher emits Antigravity sidecar activity events with error for… (+16 more)
...
Cohesion: 0.08
Nodes (23): AsyncHTTPServer, Pure asyncio HTTP/1.1 Server. Zero external dependencies, minimal memory…, Gracefully stop the HTTP server and close active connections., _make_mock_jwt(), Verify dashboard route authentication via Cloudflare Access JWT assertion…, Verify dashboard route authentication via HTTP Basic Auth and Token fallback: -…, Verify handle_root and handle_dashboard in observability.py render the styled…, test_dashboard_authentication_basic_and_token() (+15 more)

### Community 231 - "._apply_pragmas"
Cohesion: 0.29
Nodes (4): Connection, Explicitly reclaim SQLite internal cache and buffer memory., Apply tuned SQLite PRAGMAs for concurrency and bounded memory., _tuned_sqlite3_connect()
...
Cohesion: 0.09
Nodes (28): compute_dedup_hash(), Compute a deterministic deduplication hash binding signature, timestamp, and…, Verify HMAC SHA-256 signature with replay defense. - Validates presence of…, verify_hmac_signature(), Adversarial challenge: negative, zero, epoch start, float, string, and…, Adversarial challenge: None, empty strings, and whitespace in header values., Adversarial challenge: alter signature at start, middle, and end., Adversarial challenge: non-hex characters injected into signature. (+20 more)

### Community 236 - "agent_activities.py"
Cohesion: 0.18
Nodes (21): discover_sentinel_conversations(), _get_brain_dir(), _get_cadence_dir(), _get_sidecar_events_dir(), _get_signals_dir(), get_task_agent_activity(), normalize_signal_data(), parse_sentinel_transcript() (+13 more)

### Community 239 - ".sweep_and_requeue_unprocessed"
Cohesion: 0.20
Nodes (5): Insert a new task associated with an event into tasks table., Query webhook_events in 'received' status that do not have an associated task…, Synthesize and insert a task for an orphaned event in 'received' status.…, Query all unprocessed tasks and events across the system. Unprocessed includes:…, Atomic recovery and rehydration of all unprocessed tasks and events: 1. Resets…
...
### Community 243 - "._handle_client"
Cohesion: 0.21
Nodes (7): Decorator for GET routes., Handle an incoming client connection with HTTP/1.1 Keep-Alive support., Serialize and write HTTPResponse to asyncio.StreamWriter., Send a quick error response and close connection., StreamReader, StreamWriter, test_http_response_factory()

### Community 244 - ".execute_task"
Cohesion: 0.18
Nodes (7): Path, Trigger macOS malloc zone pressure relief to keep process RSS strictly < 30MB., Continuous consumer loop dequeuing tasks from queue., Persist output chunk to database., Broadcast state transition to broker., Execute a task through atomic CAS state progression: received/queued -> running…, Record observable activity event into Antigravity sidecar_data store so…

### Community 245 - "dashboard.py"
Cohesion: 0.20
Nodes (11): Any, Antigravity Webhook Hub — Observable Activity Web Dashboard Real-time web…, Generate the self-contained SPA HTML string for the Webhook Hub dashboard…, Render a premium dark-mode 401 Unauthorized page adhering to /ui-ux-pro-max…, render_dashboard_html(), render_unauthorized_html(), Any, Generate the self-contained SPA HTML string for the Webhook Hub dashboard. (+3 more)

### Community 246 - "Namespace"
Cohesion: 0.22
Nodes (11): cmd_dashboard(), cmd_rerun(), cmd_tasks(), Handle `webhook-hub dashboard` (or `ui`) command., Handle `webhook-hub tasks` command to list, inspect, and review tasks., Handle `webhook-hub rerun <task_id>` or `webhook-hub rerun --failed` command., is_test_task(), Determine if a task record is an automated, synthetic, or verification test… (+3 more)

### Community 247 - "test_agent_activities_routes.py"
Cohesion: 0.31
Nodes (10): asyncio, Tests for Antigravity Webhook Hub — Agent Activities & Sentinel Observability…, test_agent_activities_summary(), test_pulses_list(), test_sentinel_detail_and_discovery_filtering(), test_sentinel_detail_not_found(), test_sentinels_list(), test_signals_list_and_filter() (+2 more)

### Community 248 - ".sweep_unprocessed_tasks"
Cohesion: 0.24
Nodes (5): Any, Enqueue task_id for asynchronous background execution with duplicate prevention., Auto-pick and recover unprocessed tasks across SQLite SSOT. Recovers: - Stale…, Background monitor for automatic unprocessed task pickup and macOS sleep/wake…, Start background task dispatcher workers, recover orphaned tasks, and launch…
...
### Community 250 - "._pressure_relief"
Cohesion: 0.29
Nodes (3): Start the HTTP server on configured host and port., Periodic background memory relief maintaining <30MB budget on macOS., Periodic background memory relief maintaining <30MB budget on macOS.

### Community 251 - "RoutePattern"
Cohesion: 0.33
Nodes (3): Resolve route for method and path. Returns: (handler, kwargs,…, Represents a parameterized route pattern (e.g. /webhook/{source})., RoutePattern

### Community 252 - "cmd_logs"
Cohesion: 0.40
...
### Community 253 - "[1.1.0] - 2026-09-09"
Cohesion: 0.67
Nodes (3): [1.1.0] - 2026-09-09, Added, Security

### Community 254 - "[1.1.1] - 2026-09-09"
Cohesion: 0.67
Nodes (3): [1.1.1] - 2026-09-09, Added, Fixed

### Community 255 - "[1.4.0] - 2026-09-11"
Cohesion: 0.67
Nodes (3): [1.4.0] - 2026-09-11, Added, Fixed

### Community 256 - "[1.5.0] - 2026-09-11"
Cohesion: 0.67
Nodes (3): [1.5.0] - 2026-09-11, Added, Fixed

### Community 257 - "[1.5.2] - 2026-09-11"
Cohesion: 0.67
Nodes (3): [1.5.2] - 2026-09-11, Added, Fixed

### Community 258 - "[1.6.1] - 2026-09-11"
Cohesion: 0.67
Nodes (3): [1.6.1] - 2026-09-11, Changed, Fixed

### Community 259 - "[1.6.3] - 2026-09-11"
Cohesion: 0.67
Nodes (3): [1.6.3] - 2026-09-11, Added, Changed

### Community 260 - "[1.7.0] - 2026-09-11"
Cohesion: 0.67
Nodes (3): [1.7.0] - 2026-09-11, Added, Fixed

## Knowledge Gaps
- **972 isolated node(s):** `TunnelConfig`, `ObservabilityConfig`, `DashboardConfig`, `_TimeValue`, `antigravity-webhook-hub` (+967 more)
...
- **Why does `DatabaseManager` connect `DatabaseManager` to `config.py`, `cmd_sweep`, `.close`, `test_db.py`, `test_cli.py`, `server.py`, `ChallengerTestRunner`, `._commit_and_shrink`, `AppConfig`, `register_observability_routes`, `cmd_status`, `cli.py`, `AsyncHTTPServer`, `._apply_pragmas`, `M5EmpiricalRunner`, `.insert_execution_log`, `.sweep_and_requeue_unprocessed`, `Namespace`, `test_agent_activities_routes.py`, `cmd_logs`?**
  _High betweenness centrality (0.064) - this node is a cross-community bridge._
- **Why does `AsyncHTTPServer` connect `AsyncHTTPServer` to `cli.py`, `StressTestRunner`, `RouteHandler`, `AppConfig`, `config.py`, `.add_middleware`, `._pressure_relief`, `M5EmpiricalRunner`, `agent_activities.py`, `DatabaseManager`, `server.py`, `._handle_client`, `dashboard.py`, `test_agent_activities_routes.py`, `.route`, `register_observability_routes`, `RoutePattern`, `AdversarialHarness`?**
  _High betweenness centrality (0.031) - this node is a cross-community bridge._
- **Why does `TaskDispatcher` connect `AppConfig` to `cli.py`, `M5EmpiricalRunner`, `DatabaseManager`, `server.py`, `test_dispatcher.py`, `.execute_task`, `test_agent_activities_routes.py`, `.sweep_unprocessed_tasks`, `register_observability_routes`, `ChallengerTestRunner`?**
...
- **Are the 7 inferred relationships involving `AsyncHTTPServer` (e.g. with `_LightweightEmailUtils` and `_LightweightSSL`) actually correct?**
  _`AsyncHTTPServer` has 7 INFERRED edges - model-reasoned connections that need verification._
- **Are the 4 inferred relationships involving `AppConfig` (e.g. with `_LightweightEmailUtils` and `_LightweightSSL`) actually correct?**
  _`AppConfig` has 4 INFERRED edges - model-reasoned connections that need verification._
