...
TEST_EVENT_SQL_FILTER = """(
    task_id LIKE '%pw_verify%'
    OR task_id LIKE '%playwright%'
...
def is_test_task(task: Optional[dict[str, Any]]) -> bool:
    ...
    # 0. Immutable synthetic prefixes (immune to parameter spoofing)
    if "pw_verify" in task_id or "playwright" in task_id or "pw_verify" in event_id or "playwright" in event_id:
        return True
